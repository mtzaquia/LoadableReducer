...
public enum LoadableMacro {
}

extension LoadableMacro: ExtensionMacro {
...
extension LoadableMacro: MemberAttributeMacro {
  public static func expansion<D: DeclGroupSyntax, M: DeclSyntaxProtocol, C: MacroExpansionContext>(
    ...
    in context: C
      ...
      if let stateDecl = member.as(StructDeclSyntax.self),
         stateDecl.name.text == "State" {
        return [AttributeSyntax(attributeName: IdentifierTypeSyntax(name: .identifier("LoadableState")))]
      }

      if let actionDecl = member.as(EnumDeclSyntax.self),
         actionDecl.name.text == "Action" {
          return [AttributeSyntax(attributeName: IdentifierTypeSyntax(name: .identifier("LoadableAction")))]
      }

      return []

//      for attribute in stateDecl.attributes {
//        guard
